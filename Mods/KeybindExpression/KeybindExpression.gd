...
var settings_ui : ExpressionSettingsUI
var KEYBIND_PREFIX : String = "kb_"

#func check_configuration() -> PackedStringArray:
	#var errors : PackedStringArray
	#if not check_mod_dependency("Mod_Keybinds", true):
		#errors.append("No Keybinds module detected.")
	#return errors
...
#
	...
	new_keybind_button.text = "New Expression"
	...
	load_default_button.text = "Reset Expressions"
...
func on_ui_change_item(action : int, item : Dictionary, old_item : Dictionary):
	if action == ExpressionChangeAction.ACTION_NAME or action == ExpressionChangeAction.KEY_BIND:
		# Updates should be fine as there is a reference to the item.
		# We only have to check if there isn't an item, then add an item.
		if action == ExpressionChangeAction.ACTION_NAME \
	...
	if action == ExpressionChangeAction.DELETE:
